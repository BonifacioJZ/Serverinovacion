...
//const User = require('./models/user')
// Settings
app.set("port", process.emit.PORT || 3000);
...
app.post("/users/test", (req, res) => {
  ...
  (user.password = user.hash(req.body.password)), user.hash(req.body.password);
  ...
  User.findById(userID, (err, userStored) => {
    ...
    userStored.remove(err => {
      if (err)
        return res
          .status(500)
          .send({ message: `Errrore al eliminar al usuario ${err}` });
...
app.post("/singin", (req, res) => {
  ...
  User.findOne({ email: req.body.email }, (err, user) => {
    ...
    if (!user)
      return res
        .status(404)
        .send({ message: `El ususario no existe`, status: false });
    if (!user.hash(req.body.password))
      return res
        .status(404)
        .send({ message: `La contraseña no coincide`, status: false });
    req.user = user;
  ...
  User.update(
    ...
    (err, user) => {
      if (err)
        return res
          .status(500)
          .send({ message: `Error al insertar datos ${err}` });
      if (user["nModified"] === 0)
        return res.status(500).send({ message: `Èrror al guardar los datos` });
...
app.get("/data/:username", (req, res) => {
  ...
  User.findOne({ username: Username }, (err, user) => {
    if (err)
      return res
        .status(500)
        .send({ message: `error al consegir los datos del ususario ${err}` });
    if (!user)
      return res.status(404).send({ message: `No se encontro al ususario` });
  ...
  User.update(
    ...
    (err, user) => {
      if (err)
        return res
          .status(500)
          .send({ message: `Error al eliminar la informacion` });
      if (user["nModified"] === 0)
        return res
          .status(500)
          .send({ message: `Error al eliminar la informacion` });
  ...
  user.save((err, us) => {
    if (err)
      return res.status(500).send({
        message: {
          msg: `Error al registrar el ususarios ${err}`,
          status: false
        }
      });
